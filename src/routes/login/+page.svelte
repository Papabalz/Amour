...
<fieldset class="fieldset bg-base-200 border-base-300 rounded-box w-xs border p-4">
    <legend class="fieldset-legend text-2xl">
        ...
        {t.login_signin()}</legend>
   <form method="POST" action="/login" use:enhance={() => {
      isLoading = true;
    } }>
    <input id="email" name="email" type="email" class="input" placeholder="{t.login_email_placeholder()}" />
  
    <label for="password" class="label">{t.login_password()}</label>
    <input id="password" name="password" type="password" class="input" placeholder="{t.login_password_placeholder()}" />
   <a href="/forgot-password" class="text-sm text-blue-500 hover:underline mt-2">{t.login_forgot_password()}</a>
    ...
    <a href="/signup" class="btn btn-link mt-4">{t.login_signup_link()}</a>
</div>
</form>
<div>
